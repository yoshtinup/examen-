...
class AsignaturaQuerys extends ApiQuerys {
  async getAgregarAsignatura(idMateriaOfertaAnual, justificacion): Promise<any> {
  ...
  async getCambioAsignatura(idMateriaOfertaAnualBaja, idMateriasOfertaAnualAlta, justificacion): Promise<any> {
    const status = await this.api<{ data: DatosMessage }>(
      `cambio_asignatura`,  {
        method: 'POST',
        body: JSON.stringify({ idMateriasOfertaAnualBaja: idMateriaOfertaAnualBaja,idMateriasOfertaAnualAlta: idMateriasOfertaAnualAlta, justificacion: justificacion }),
        headers: {
          'Content-Type': 'application/json'
        }
      }
