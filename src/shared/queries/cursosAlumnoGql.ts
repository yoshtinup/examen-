      ...
      const data = await hasuraClient.request(
        gql`
          query CursosAlumno($matricula: Int!) {
            Cursos: db12_view_BoletaInscripcion_Matricula_Periodo(
              where: { Matricula: { _eq: $matricula } }
              order_by: { FechaIni: asc }
            ) {
              NombreMateria
              Clave
              Creditos
              ...
              CategoriaMateria: db12_MateriasOferataClave {
                Datos: db12_ClaveMaterium {
                  Value: CategoriaMateria
                }
              }
              ...
              EvaluacionDocentePendiente: db_EvaluacionDocenteMateria(
                where: { EstatusEvaluacion: { _eq: 3 } }
              ) {
            ...
            CreditosAsignatura: CnsEstudiante_CreditosAsignaturas_Cursadas(
              where: { Matricula: { _eq: $matricula } }
            ) {
              AsingaturasCursadas
              TotalDeCreditos
