        ...
        {isLoading ? (
          ...
          <TablaAsignaturas
            asignaturas={data.Asignaturas.filter(
              (item: AsignaturaGql) => (Date.parse(item.Datos.FechasAsignatura.FechaInicio) < Date.now() && Date.parse(item.Datos.FechasAsignatura.FechaFin) > Date.now())
            ).sort(
...
export default AsignaturasEnProceso;
