...
import { AsignaturaGql, FechasAsignaturaGql } from '@shared/types/listaAsignaturas';
...
const columns: GridColDef[] = [
  ...
  { field: 'opciones', headerName: 'Opciones', sortable: false, width: 130,
    renderCell: (params) => {
      return (
        <Button
  ...
  return (
    <Grid item xs={12}>
      ...
      <Box style={{marginBottom: "30px"}} >
        ...
        {programa > 0 &&
          <CrearSelect nombre="periodos" data={periodo} setData={setPeriodo} opciones={Periodos[programa]} />
        }
      </Box>
...
function CrearSelect({nombre, data, setData, opciones} : {nombre: string, data: number, setData: any, opciones: any}){
  const nameFirstUpper = nombre.charAt(0).toUpperCase() + nombre.slice(1);
  return (
    <FormControl style={{width: "25%", margin: "0 4%"}}>
      <InputLabel id={"asignaturas-" + nombre + "-select-label"}>{nameFirstUpper}</InputLabel>
      <Select
      ...
      >
        {opciones.items.map((unidad: any, i: number) =>
          <MenuItem key={i} value={unidad.value}>{unidad.label}</MenuItem>
        )}
      </Select>
    </FormControl>
  );
...
function getRows(listaAsignaturas: AsignaturaGql[], unidad = 0, programa = 0, periodo = 0){
  let rows = [];
  listaAsignaturas.forEach(asignatura => {
    const contieneUnidad: boolean = (unidad > 0) ? (asignatura.Unidad.Id == unidad) : true;
    const contienePrograma: boolean = (programa > 0) ? (asignatura.Asignatura.Datos.Programa.Id == programa) : true;
    const contienePeriodo: boolean = (periodo > 0) ? (asignatura.Datos.Periodo.Id == periodo) : true;
...
function filtros(listaAsignaturas: AsignaturaGql[]){
  let Unidades = { ids: [0], items: [ { value: 0, label: "Todas las unidades" } ] };
  let Programas = { ids: [0], items: [ { value: 0, label: "Todos los programas" } ] };
  let Periodos = [ { ids: [0], items: [ { value: 0, label: "Todos los periodos" } ] } ];
  listaAsignaturas.forEach(asignatura => {
    ...
    if(!Periodos[asignatura.Asignatura.Datos.Programa.Id]){
      Periodos[asignatura.Asignatura.Datos.Programa.Id] = { ids: [0], items: [ { value: 0, label: "Todos los periodos" } ] };
    }
    if(!Periodos[asignatura.Asignatura.Datos.Programa.Id].ids.includes(asignatura.Datos.Periodo.Id)){
      Periodos[asignatura.Asignatura.Datos.Programa.Id].ids.push(asignatura.Datos.Periodo.Id);
...
function formatoFecha(fechas: FechasAsignaturaGql){
  ...
  return fechaIni.getDate() + " de " +
    meses[fechaIni.getMonth()] + " del " +
    fechaIni.getFullYear() + " al " +
    fechaFin.getDate() + " de " +
    meses[fechaFin.getMonth()] + " del " +
