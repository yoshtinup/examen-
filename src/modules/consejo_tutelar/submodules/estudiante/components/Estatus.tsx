...
export default function Estatus({ matricula }: { matricula: number }) {
  const { isLoading, isSuccess, error, data } = useGetEstudianteCTEstatus(matricula);
  if (isLoading) <CircularProgress />;
  if (error) <Alert>No se pudo cargar el estatus</Alert>;
  return (<>{ isSuccess && <EstatusProcesoCT estatus={data[0].Estatus.Leyenda} />}</>);
}
