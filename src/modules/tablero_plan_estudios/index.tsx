...
const TableroPlanEstudios = () => {
  ...
  const listaCursos = useGetCursosAlumno(user.estudiante.matricula);/*202211001*/
  ...
  const arrayCursosAIniciar:CursoPorIniciarGql[] = crearJSON2().data.sort((a, b) => {
  ...
  return (
    <>
      ...
      <EcosurTabs data={tabCursos} align='left' activeColor="#ecf0f5" activeTextColor="black" key='ecosur-tabs-cursos' />
    </>
  );
};
...
function getCuatrimestresSemestres(listaCursos:any){
  ...
  listaCursos?.data?.forEach((curso:CursoGql) => {
    if(!ids.includes(curso.IdPeriodo)){
      ...
      currentCS.CalificacionPendiente = curso.CalificacionNumerico ? currentCS.CalificacionPendiente : true;
    }
  });
  CuaSem.forEach(CS => {
  ...
  return {
    data: [
      ...
      },
      {
        ...
        NombreMateria: "Ecología del Pastoreo y Ramoneo en Sistemas Ganaderos Tropicales",
      ...
      },
      {
        ...
        NombreMateria: "Fundamentos de percepción remota y ecología del paisaje",
      ...
      },
      {
        ...
        NombreMateria: "Sexualidad y Salud Reproductiva desde el enfoque feminista de género y derechos humanos",
      ...
      },
      {
        ...
        NombreMateria: "Género y otras desigualdades en la Frontera Sur de México",
